...
tar_load(bird_singleton_summary, bird_summ)
tar_load(testate_summ)
tar_load(last_chance_summ)
tar_load(pollen_summ, hershop_pollen_summ, wildhorse, pollen_summ1, pollen_summ2, pollen3, pollen1, pollen2)
tar_load(owens_summ1, owens_summ, owens_est_summ)
tar_load(diatom1_summ, diatom2_summ, chironomid1_summ, marine1_summ, chironomid2_summ)
...
5) Fossil pollen assemblage from the Neotoma database [@neotoma_2018] downloaded on `r tar_read(pollen_download_date)`.
...
tar_read(bird_recount_singleton_plot)
```
