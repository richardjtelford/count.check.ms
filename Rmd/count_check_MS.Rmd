...
loadd(pollen_summ, hershop_pollen_summ, pollenA, pollen1234)
...
For percent data, the count sum was estimated using the minimum percent method and the direct search methods described above. 
...
drake v. `r packageVersion("drake")` [@drake], 
...
Discrepancies between the archived data and the original paper [@Larson_1972] including an incomplete species list and the inclusion of fern spores in the count sum of two hundred grains strongly suggest that these data have been digitised.
For most purposes, such digitised data are perfectly adequate.

Excluding such datasets would further increase the proportion of assemblages with singletons. 
Some other Neotoma datasets are discussed in section \@ref(unexpected-count-data) of this paper.  
...
Here I focus on five anonymised datasets. 

Dataset pollen1, published includes `r pollenA$n_samp` assemblages with a median richness of `r pollenA$taxa_median` taxa.
Count sums vary between `r pollenA$count_min` and `r pollenA$count_max`.
The greatest common divisor is three for all assemblages: all `r pollenA$n_count` counts in the dataset are divisible by three. 
Assuming (optimistically) the distribution of counts is (at least locally) uniform, the probability of _n_ counts being divisible by _k_ is $1/k^n$.
For a dataset this large, the probability of having all counts divisible by three by chance is 10^`r ceiling(- pollenA$n_count * log10(3))`^.
Given this very small probability, it seems reasonable to conclude that the archived data are not the original counts, but I have no idea how they were generated.
