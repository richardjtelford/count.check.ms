...
bookdown v. `r packageVersion("bookdown")` [@bookdown1] and
...
At the order level, the proportion of counts without singletons decreases steeply until it reaches a minimum at when the count sum is about 200, thereafter it increases slowly (Fig. \@ref(fig:bird-recount)).
...
```{r bird-recount, fig.cap="Effect of count sum on the proportion of bird counts from the North American breeding bird survey without singleton at the order level. Results are the mean of ten trials with 95% confidence interval.", fig.width = 80/25.4, fig.height = 70/25.4}
...
```{r percent-est-plot, fig.cap = "Accuracy of estimates of the count sum calculated with two different methods with percent data have been rounded to different numbers of digits. Percent data are simulated by resampling a bird data route with 30 species to different count sums.", fig.width = 80/25.4, fig.height = 80/25.4}
readd(percent_rounding_plot)
```
