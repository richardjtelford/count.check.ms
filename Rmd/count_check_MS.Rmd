...
Count data analysed include the North American Breeding Bird survey [@bbsAssistant2]; 
the Brown University planktic foraminiferal database [@Prell1999];
the MOLTEN diatom database [@Elinor]; 
the North American testate amoeba training set [@AMESBURY2018];
and fossil pollen assemblage from the Neotoma database [@neotoma_2018]. 
Pollen data were downloaded on `r readd(pollen_download_date)`, samples with with small count sums (< 50), that appeared to be percentages, or were documented as back-transformed from digitised data were excluded. 
...
To test for rare taxa, the prevalence of samples without singletons was calculated for each data set.
...
The North American breeding bird survey is a very large, annual monitoring programme to track the status of North American bird populations [@bbsAssistant2].
...
The vast majority of the bird routes have singletons at the species level.
Only `r bird_summ$n_without_singletons` routes do not have singletons, of these only `r bird_summ$n_GCD_gt1 %>% nrow() %>% words()`, all with very low richness, have a greatest common divisor greater than one.

To explore the effect of taxonomic richness on the prevalence of singletons, I aggregate the data to progressively lower taxonomic resolutions.
...
To test the sensitivity of the prevalence of routes without singletons to the count sum, I resample the routes with 400 or more individuals to smaller count sums using a multivariate hypergeometric distribution.
...
The ` nrow(owens_summ1)` diatom counts from Owen's Lake span a wide range of count sums (` min(owens_summ1$count_sum)` -- ` max(owens_summ1$count_sum)`) because, if concentrations were low, the requirement that at least 300 valves were counted was replaced by a minimum area of slide to count [@Platt1997].
Discounting the ` sum(owens_summ1$n_taxa == 1)` mono-specific assemblages (all with very low count sums ` max(owens_summ1$count_sum[owens_summ1$n_taxa == 1]) %>% insist(. <= 10) %>% invisible()`), ` owens_summ$p_singletons`% of assemblages have singletons with a median number of singletons per assemblage of ` owens_summ$median_sing %>% words()`.
Assemblages with count sums above 50 have a higher chance of having singletons (` owens_summ$no_single_gt_50`% vs ` owens_summ$no_single50_`%).
` owens_summ$gcd2`% of assemblages have a greatest common divisor of one, the remainder have a greatest common divisor of ` owens_summ$gcd_max %>% words()`.
Assemblages with a greatest common divisor above one all have low taxonomic richness` owens_summ1 %>% filter(gcd > 1) %>% pull(n_taxa) %>% max() %>% insist(. <= 6) %>% invisible()`.
...
options(digits = 4)
readd(count_summary) %>% 
  knitr::kable(caption = "Properties of different data sets")
options(digits = 2)
```
...
The Owen's Lake dataset [@Platt1997] includes percentages for each taxa, allowing the count sums estimated from the percent data to be verified. Excluding mono-specific assemblages, for which no meaningful estimate of the count sum is possible, the minimum percent method and the direct search method correctly estimate the count sum for, respectively, ` owens_est_summ$minpc`% and ` owens_est_summ$direct`% of the assemblages. 
