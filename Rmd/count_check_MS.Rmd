...
Assuming (optimistically) the distribution of counts is (at least locally) uniform, the probability of _n_ counts being divisible by _k_ is $k^{-n}$.
...
In datasets pollen2 and pollen3, presented in the same paper, # % and # % of assemblages do not have singletons and have a greatest common divisor greater than one.
Both datasets match the published pollen stratigraphies.
For most of both datasets, assemblages with and without singletons are arranged alternately.
Samples without singletons are on the 10 cm intervals, and those with on the intervening 5 cm intervals. 
...
In the published and archived data, they had been interpolated from their neighbours (Fig. \@ref(fig:archived-original-plot-plot)).

fig - plot of 1 or 2 mid-common taxa by depth in archived and original 
...
```{r archived-original-plot, fig.height = 70/24.5, fig.width=80/24.5, fig.cap="Number of taxa in against count sum for assemblages in two pollen datasets with a greatest common divisor of one (red circles) or two (blue triangles). Lines are loess with a span of 1. All four pollen datasets show the same pattern, but it is clearest in datasets 1 and 2 which have the highest proportion of assemblages with a greatest common divisor of two.", message = FALSE}
readd(archived_original_plot)
```
