...
# Packages loaded for convenience when knitting without running r_make
library("drake")
library("tidyverse")
library("rjt.misc")
...
loadd(bird_singleton_summary, bird_summ)
loadd(testate_summ)
...
The ubiquitous shape of species abundance distributions when plotted as the number of species against abundance is a hyperbolic or hollow curve with many rare and few abundant species and is central to ecological theory [@McGill_2007].
From this, two predictions can be made: that counts of an entire community or assemblage should have many rare taxa, and that the distribution of abundances should be smooth.
Count data that differ markedly from these predictions might not accurately reflect the original community or assemblage.

This paper aims to develop and test simple methods to test the abundance of rare taxa and the smoothness of the species abundance distribution that can  use to flag cases if the count data might have been misreported.
...
All birds seen in point counts along ~40 km survey routes at the height of the breeding season are recorded. 
...
5) Fossil pollen assemblage from the Neotoma database [@neotoma_2018] downloaded on `r readd(pollen_download_date)`.
...
The prevalence of counts without singletons, and counts with a greatest common divisor greater than one, was calculated for each data set.

To test if the species abundance distribution is smooth, I used two tests.
First, for each assemblage or community with at least ` word(min_richness)` taxa, the proportion of taxa with counts divisible by $k$ where $k$ is a small integer larger than one is calculated.
Samples where this proportion is high have a saw-toothed pattern of counts. 
In extreme cases, where all taxa have counts divisible by $k$, the sample has a greatest common divisor of $k$.
Data sets with a many samples with a greatest common divisor greater than one or an unusually high proportion, relative to the majority of pollen data set in the Neotoma database, of counts divisible by $k$ were flagged for closer inspection.

The second test of smoothness is to fit a generalised additive model with Poisson errors to the counts aggregated over samples in each data set. 
Only counts of ` word(gam_max)` or fewer were considered due to low power in the relatively infrequent larger counts.
Data sets where the proportion of explained deviance is low relative to the majority of pollen data set in the Neotoma database were flagged for closer inspection.
...
Across all the datasets, only a small minority of samples lack singletons, and even fewer have a greatest common divisor larger than one (Table \@ref(tab:multitaxa).
Most of the samples without singletons, or with a greatest common divisor larger than one, have low richness (Fig. \@ref(fig:richness-singleton-plot).

Although the proportion of pollen assemblages that lack singletons or have a greatest common divisor of one are low, such assemblages are not randomly distributed among the `r pollen_summ$n_datasets %>% format()` pollen datasets.

Half of the assemblages without singletons are in just `r pollen_summ$half_no_sing` datasets.
`r Words(pollen_summ$half_gcd)` datasets contain half of the assemblages with a greatest common divisor above one and only `r pollen_summ$dataset_gcd1`% of the datasets have any such assemblages.
Some pollen datasets with unexpected properties are discussed in section \@ref(unexpected-count-data).  
...
readd(count_summary) %>% 
  ...
  kbl(x = ., booktabs = TRUE, escape = FALSE,
      caption = "Properties of different data sets", 
      col.names = linebreak(str_wrap(names(.), width = 10))
    ) %>% 
  kable_styling(latex_options = c("basic", "hold_position"))
```

```{r richness-singleton-plot, fig.cap = "Relationship betwen the presence of singletons and taxonomic richness for the different datasets. Samples with a greatest common divisor greater than one are shown in dark blue. Curve is fitted with a generalised additive model", fig.height = 120/24.5, fig.width=160/24.5}
readd(richness_singleton_plot)
```
...
As richness declines, the proportion of counts without singletons increases (Table \@ref(tab:multitaxa)), reaching a moderate proportion at the order level, where most of the counts lacking singletons have fewer than five taxa.
...
The precision of both methods for estimating the count sum from percent data depend on count sum and on the precision to which the percent are recorded (Fig. \@ref(fig:percent-est-plot).
...
```{r percent-est-plot, fig.cap = "Accuracy of estimates of the count sum calculated with two different methods with percent data have been rounded to different numbers of digits, based on some simulated data", fig.width = 80/25.4, fig.height = 80/25.4}
readd(percent_rounding_plot) + theme(legend.position = "bottom")
```
...
```{r interpolated-plot, warning = FALSE, fig.height = 100/24.5, fig.width=180/24.5, fig.cap="A) Plot of selected taxa by depth from dataset pollen2. Counts with blue triangles are from assemblages with a greatest common divisor of one. These have been linearly interpolated between the adjacent counts from assemblages with a greatest common divisor of two (red circles). B) Percent of taxa in each assemblage where the count is the mean of the counts in the adjacent assemblages, excluding runs of zero counts. Most of the assemblages with a greatest common divisor of one are exactly linearly interpolated between adjacent assemblages. The main exception is the assemblage at 210 cm which has a single singleton and otherwise has a greatest common divisor of two, other exceptions have a few extra singletons not predicted from the adjacent assembages.", message = FALSE}
readd(pollen4_plot)
```
